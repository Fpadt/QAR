...
pDAY <- today()
dtCR <- fread(file.path(RAW, paste0("CR_W", isoweek(pDAY), ".csv"))) 
dtCD <- fread(file.path(RAW, paste0("CD_W", isoweek(pDAY), ".csv"))) 
...
shell.exec(file.path(normalizePath(RES), paste0(pXLSX, ".xlsx")))
